...
<nav class="navbar navbar-light "  style="background-color:#837d7d">
 ...
 <a class="btn btn-success mt-3 m-3 w-100"  href="done.php">تم التسليم  </a>
...
<nav class="navbar navbar-light bg-light">
  <form class="container-fluid">
    <div class="input-group">
      <input class=" form-control " aria-describedby="basic-addon1" style="text-align: center;" autofocus placeholder="تسليم للعميل" required type="number" name="nmuber" >
      <button  class="input-group-text btn btn-outline-success " id="basic-addon1" type="submit" name="send"> تسليم </button>
...
<form class="container-fluid">
    <div class="input-group">
<input class=" form-control " aria-describedby="basic-addon1"  style="text-align: center;" type="number" name="searchv" placeholder="البحث عن العناصر تم  تسليمها "  >
...
header( "Refresh:30;url=http://localhost/out/done.php" ); 
...
if(isset($_GET['send'])){
  ...
  if($addcheak->rowCount()>0){
    ...
    if($addcheak->STATA =="DONE"){
      ...
      تم تسليمه للعميل بتاريخ '.
      $addcheak->DONE_DATE .'
      
      </div>';
    }else if ($addcheak->STATA =="IN SHOP"){
      $UBDATE = $databass->prepare("  UPDATE `shop` SET `STATA`='DONE', DONE_DATE=CURRENT_TIMESTAMP WHERE NUMBER = :id");
      $UBDATE->bindParam('id',$_GET['nmuber']);
       $UBDATE->execute();

       echo' <div class="alert alert-success" role="alert">
       تم التسليم للعميل    </div>';   
       
       
     
    }else if($addcheak->STATA =='IN LAP'){
      echo' <div class="alert alert-success" role="alert">
      لم يم استلامه من المصنع   </div>';  
    }
...
// header("Refresh:30; url=done.php");
}



    
$view=$databass->prepare("SELECT * FROM shop WHERE STATA ='DONE'" );
if($view->execute()){
    if($view->rowCount()>0){
        echo '<table class="table mt-3">';
        echo   " <tr  class='table-primary'>";
